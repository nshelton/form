...
Shader "SHELTRON/FractalShader" {
    Properties {
        ...
        _StepRatio ("stepRatio", Range(0,10)) = 0.5
        _palette ("palette", Range(0,1)) = 0.2
			_scale("scale", Range(0,1)) = 1.0
			_fovea("fovea", Range(0,1)) = 1.0
			_fractalType("fractalType", Range(0,1)) = 1.0
...
float query(float3 pos)
{
	...
	if (_fractalType < 0.33)
		d = min(d, DE(pos));
	else if (_fractalType < 0.666)
		d = min(d, MBOX(pos));
	else
		d = min(d, hartverdrahtet(pos));
...
void frag (v2f inV2F, out fixed4 col : SV_Target, out float outDepth : SV_Depth)

{
    float4 cam_pos = float4(_WorldSpaceCameraPos, 1.0);
    cam_pos =  mul(_WorldToModelTransform, cam_pos);

    float4 dir = float4(normalize(inV2F.n - _WorldSpaceCameraPos), 0.0);
    dir = mul(_WorldToModelTransform, dir);

	float fovea = length(inV2F.uv);
	fovea = exp(-fovea*fovea / _fovea) *0.5;
    ...
    for ( int i = 0; i < ITERS; i ++)
    {
        ...
        float thr = exp(-_Threshold*2.0) * (1.0 - fovea) * exp(_StepRatio * depth   )  ;
    ...
    if (hit)
    {
        ...
        col = tex2Dlod(_ColorMap, float4(_palette, 1.0 -ao, 0.0, 0.0));
        col.xyz  *= ao ;
       // if ( hit)
